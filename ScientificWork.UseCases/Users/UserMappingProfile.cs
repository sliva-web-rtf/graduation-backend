...
public class UserMappingProfile : Profile
{
    ...
    public UserMappingProfile()
    {
        ...
        CreateMap<Student, GetStudentOnBoardingInfoCommandResult>()
            ...
            .ForMember(x => x.Status, opt => opt.MapFrom(student => student.SearchStatus.Status.ToString()))
            .ForMember(x => x.CommandSearching, opt => opt.MapFrom(student => student.SearchStatus.CommandSearching))
            .ForMember(x => x.ProfessorSearching,
                opt => opt.MapFrom(student => student.SearchStatus.ProfessorSearching));

        CreateMap<Professor, GetProfessorOnBoardingInfoCommandResult>()
            .ForMember(x => x.ScientificArea, opt => opt.Ignore())
            .ForMember(x => x.SearchStatus, opt => opt.MapFrom(student => student.SearchStatus.Status.ToString()))
