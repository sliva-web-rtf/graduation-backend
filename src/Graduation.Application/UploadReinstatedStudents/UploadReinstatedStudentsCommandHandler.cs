...
public class UploadReinstatedStudentsCommandHandler : IRequestHandler<UploadReinstatedStudentsCommand>
{
    ...
    public UploadReinstatedStudentsCommandHandler(ISender sender, UserManager<User> userManager, IAppDbContext dbContext, ICurrentYearProvider currentYearProvider)
    {
        this.sender = sender;
    ...
    public async Task Handle(UploadReinstatedStudentsCommand request, CancellationToken cancellationToken)
    {
        ...
        for (var i = 2; i <= countRow; i++)
        {
            ...
            var studentsGroup = ws.Cell($"A{i}").GetValue<string>();

            
            if (string.IsNullOrWhiteSpace(fullName) || string.IsNullOrWhiteSpace(studentsGroup))
            {
                continue;
            }
            ...
            if (academicGroup == null)
            {
                ...
                {
                    Name = studentsGroup, Year = currentYearProvider.GetCurrentYear()
                };
