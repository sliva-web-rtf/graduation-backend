...
public class CommissionStudentConfiguration : IEntityTypeConfiguration<CommissionStudent>
{
    public void Configure(EntityTypeBuilder<CommissionStudent> builder)
    {
        ...
        builder.HasOne<Student>().WithMany(s => s.CommissionStudents).HasForeignKey(x => x.UserId);
        builder.HasOne<Commission>(cs => cs.Commission).WithMany().HasForeignKey(x => x.CommissionId);
